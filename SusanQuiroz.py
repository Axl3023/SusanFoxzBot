...
async def logUserBot():
    load_dotenv()
    api_id = int(28626179)
    api_hash = "ce9066923ae951dedafbbef7e26339a2"
    phone_number = "51921808502"
    ...
    await client.send_message("@SpamSusan", f'<b>Bot encendido</b>', parse_mode="HTML")
    ...
    async def handler(event):
        ...
        if event.is_private and sender.bot is False:  # Asegurarse de que no sea un bot ni un grupo
            await client.send_message(sender.id, 
                                      "Hola, Soy bot oficial de Spam de @Cmdfoxz escribeme y te atendere al instante.\n@Cmdfoxz")
    
    spammer_group = int("-4747497664")
    ...
    while True:
        ...
        try:
            await client.send_message("@SpamSusan", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024", "RESPALDO🇵🇪BINS PERU", "➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧", "QUEMANDO ESTAFADORES", "𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀", "Referencias Elmer 💸", "🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪", "TU MARKETPLACE", "⚫️𝙈𝙀𝙍𝘾𝘼𝘿𝙊 𝙉𝙀𝙂𝙍𝙊⚫️", "💻 BLAK PERU OFC COMUNITY 💻", "🇵🇪PRIMITOS BINS PERU🇵🇪"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@SpamSusan", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                            ...
                            if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                ...
                                try:
                                    if message_spam.text:  # Verifica si el mensaje tiene texto
                                        ...
                                        await client.send_message("@SpamSusan", f'<b>Multimedia no permitido en {i["group_name"]}. Se envió solo texto.</b>', parse_mode="HTML")
                                    else:
                                        await client.send_message("@SpamSusan", f'<b>No se pudo enviar a {i["group_name"]}: el mensaje no tiene texto válido.</b>', parse_mode="HTML")
                                except Exception as inner_error:
                                    await client.send_message("@SpamSusan", f'<b>Error enviando texto a {i["group_name"]}: {inner_error}</b>', parse_mode="HTML")
                            else:
